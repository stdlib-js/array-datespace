...
function validDate( value, name ) {
	...
	if ( type === 'string' ) {
		...
		if ( value !== value ) {
			throw new Error( format( '00b2R', name.toLowerCase() ) );
		}
		value = new Date( value );
	}
	if ( type === 'number' ) {
		if ( !timestamp.test( value ) ) {
			throw new Error( format( '00b2S', name.toLowerCase() ) );
		}
	...
	if ( !(value instanceof Date) ) {
		throw new TypeError( format( '00b2T', name ) );
	}
	return value;
}
...
function datespace( start, stop, length, options ) {
	...
	if ( arguments.length > 2 ) {
		...
		if ( !isInteger( len ) || len < 0 ) {
			throw new TypeError( format( '00b2U', len ) );
		}
		if ( flg ) {
			if ( !isObject( opts ) ) {
				throw new TypeError( format( '00b2V', opts ) );
			}
			if ( hasOwnProp( opts, 'round' ) ) {
				if ( !isString( opts.round ) ) {
					throw new TypeError( format( '00b2W', 'round', opts.round ) );
				}
				if ( rounders.indexOf( opts.round ) === -1 ) {
					throw new Error( format( '00b4S', 'round', rounders.join( '", "' ), opts.round ) );
				}
			}
		}
	}
