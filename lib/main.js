...
function validDate( value, name ) {
	...
	if ( type === 'string' ) {
		...
		if ( value !== value ) {
			throw new Error( format( '00I2d', name.toLowerCase() ) );
		}
		value = new Date( value );
	}
	if ( type === 'number' ) {
		if ( !timestamp.test( value ) ) {
			throw new Error( format( '00I2e', name.toLowerCase() ) );
		}
	...
	if ( !(value instanceof Date) ) {
		throw new TypeError( format( '00I2f', name ) );
	}
	return value;
}
...
function datespace( start, stop, length, options ) {
	...
	if ( arguments.length > 2 ) {
		...
		if ( !isInteger( len ) || len < 0 ) {
			throw new TypeError( format( '00I2g', len ) );
		}
		if ( flg ) {
			if ( !isObject( opts ) ) {
				throw new TypeError( format( '00I2h', opts ) );
			}
			if ( hasOwnProp( opts, 'round' ) ) {
				if ( !isString( opts.round ) ) {
					throw new TypeError( format( '00I2i', 'round', opts.round ) );
				}
				if ( rounders.indexOf( opts.round ) === -1 ) {
					throw new Error( format( '00I3t', 'round', rounders.join( '", "' ), opts.round ) );
				}
			}
		}
	}
